...
int cs_init(
    ...
    if (!s || !r_xyz || !V || !vol) return 1;
    if (n == 0) return 2;
    if (!(M > 0.0)) return 3;
    if (softening < 0.0) return 4;
    ...
    s->exclude_self = exclude_self ? 1 : 0;
...
void cs_chi_from_masses(const conformal_solver_t *s, const double *m, double *tilde_chi_next_out) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (size_t j = 0; j < n; ++j) {
            if (s->exclude_self && eps2 == 0.0 && j == i) continue;
...
int cs_solve(
    ...
    if (!s || !psi_out || !info_out) return 1;
    if (!(tol > 0.0)) return 2;
    if (max_iter <= 0) return 3;
